...
export function AuthPage() {
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post("/api/otp/send", { phone: formData.phone });
      setOtpSent(true);
      setCountdown(60);
      setErrors({});
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (authMode === 'password') {
        response = await api.post("/api/auth/admin/login", formData);
      } else {
        if (!otpSent) {
          await handleSendOtp();
          return;
        }
        response = await api.post("/api/otp/verify", { 
      ...
      setErrors({ general: error.response?.data?.message || error.response?.data?.error || "Login failed. Please try again." });
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="flex bg-gray-100 rounded-lg p-1 mt-4">
              <button
                ...
                onClick={() => {
                  ...
                  setOtpSent(false);
                  setErrors({});
                  setFormData(prev => ({ ...prev, otp: '', phone: '' }));
                }}
